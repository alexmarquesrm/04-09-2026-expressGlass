...
94 tests pass, including HTTP-level tests for the column routes written from the start this time (the S5 lesson), and tests for the confirm flow's cross-user rejection and single-use claim. Verified live in a real browser end-to-end: login, add a column, add a card into it, rename the column inline, move the card out via the card's column control, open the assistant panel, delete the column, delete the card. No assistant message was actually sent, because a real DeepSeek key is configured and each call costs money.
