...
**Second follow-up: what real use immediately broke.** The first live conversation against the assistant (the user's own, spending real API calls) found two things that no amount of service-level testing would have:

- **"tarefas pendentes nos quadros" failed outright** with a generic "não foi possível falar com o assistente". The backend log had the real cause: `400 An assistant message with 'tool_calls' must be followed by tool messages responding to each 'tool_call_id'`. `handleMessage` only ever read `tool_calls[0]` and pushed a single tool reply — but the provider batches calls (here, one `list_board_tasks` per board), so the *next* request in the loop was malformed and the whole turn died. This was latent in the original M4 chat code and unreachable while the assistant only had four single-target task tools; boards made it routine. Fixed by executing every call in the response and replying to each `tool_call_id`. A destructive call still ends the turn immediately, which is safe precisely because the conversation doesn't continue from there.
- **Asking about "board tasks" ignored what was assigned to the person asking.** Added `list_my_board_tasks`: every card assigned to the caller across all their boards, carrying `board_name` and `column_name`, answered in one call instead of the model walking each board (which is what pushed it into the batched-call path in the first place). The `board_members` join matters — being removed from a board hides its cards even if the assignment row survives. The system prompt now also tells the model to say *where* results came from, since `list_tasks` (personal tasks) and the board cards are separate lists and the distinction was invisible in the replies.

The multi-call bug got a genuine regression test: `handleMessage` now takes an injectable client, so a fake provider can return two tool calls in one response and the test asserts both `tool_call_id`s are answered — without spending an API call.

**Third follow-up: several people per card.** `assignee_id` became a `board_task_assignees` join table (migration 013), backfilled from the old column, which is then dropped — keeping both would have been the same dual-truth trap that `status`/`column_id` already showed. Cards now carry an `assignees` array of `{user_id, name}`, `assignee_ids` replaces the whole set on write (validated so a single non-member rejects the entire assignment, inside a transaction so nothing is half-written), and `list_my_board_tasks` joins through the new table. The assistant's confirmation diff names every person on both sides of the change rather than printing raw ids.

In the UI, the modal shows a chip only for people actually **on** the card, each with its own remove control, plus a round `+` that opens a picker listing every board member with a check against those already chosen — a board with many members would otherwise turn the modal into a wall of names. Card faces show an avatar stack. Verified in a browser: three people onto one card, an avatar each after a reload, removing one leaves the rest, clearing all leaves none, and drag still leaves nothing greyed out. One real find from that run: the modal's remove buttons had the same `aria-label` as the board's Membros panel remove buttons, so "Remover Bruno Santos" was ambiguous on the page — fixed in the component ("Remover X do cartão") rather than by narrowing the test selector.

99 tests pass. No assistant message has been sent from this side at any point, because a real DeepSeek key is configured and each call costs money — the tool layer is exercised directly instead, which is also why the batched-tool-call bug survived until a human actually talked to it.

## 13. Frontend tests + CI (closing the biggest remaining gap)

A holistic "what's missing" review flagged the same thing every earlier feature-level review had no reason to notice: the backend had 99 tests and the frontend had none, and nothing ran either suite automatically. Every UI check up to this point was a manual or throwaway Playwright session — real verification, but nothing a reviewer (or a future change) could re-run for free.

Added Vitest + React Testing Library (`frontend/vitest.config.js`, jsdom environment, `frontend/src/test/setup.js` for jest-dom matchers) and 23 tests across the components most worth protecting: `color.js` (the avatar gradient/initials pure functions), `TaskForm`, `TaskList` (including the confirm-before-delete dialog and inline edit), and `TaskModal` — the newest and most stateful piece, covering the assignee picker (chips only for assigned members, `+` opens a picker of everyone, add/remove both round-trip into `assignee_ids` on save), colour-label toggling, and the disabled-save-on-empty-title guard. `npm test` runs `vitest run` inside the frontend container the same way `npm test` already ran the backend's `node --test`.

Added `.github/workflows/ci.yml`: two independent jobs, `backend` (a `postgres:16-alpine` service container, `npm ci` → `npm run migrate` → `npm test`, throwaway CI-only `DATABASE_URL`/`JWT_SECRET`) and `frontend` (`npm ci` → `npm test`), on every push to `main` and every pull request, with an explicit `permissions: contents: read` (Security's one suggestion — the workflow never needs write access to anything).

Security and Review-QA both ran against this slice too, small as it is:

- Security found nothing above a hygiene suggestion (the `permissions:` block above) — no fork-PR secret exposure (the trigger is plain `pull_request`, not `pull_request_target`, and no secrets are referenced anyway), no injection via PR title/branch interpolation, and the hardcoded CI-only `JWT_SECRET`/Postgres password are fine since they never touch anything but the job's own throwaway service container.
- Review-QA found one real product bug hiding behind a wrongly-worded test: `TaskForm.test.jsx` was titled "submits the trimmed title" but asserted the *untrimmed* value, because `TaskForm.jsx` only ever trimmed `description`, never `title` — inconsistent with `TaskModal.jsx`, which trims both. Fixed the component (`title.trim()` before calling `onCreate`) and rewrote the test to actually assert the trimmed value and a real due date. QA also flagged real coverage gaps for a first pass: `TaskModal`'s Escape-to-close and click-outside-to-close, its Eliminar button, saving a changed column/priority, toggling a label back off; `TaskForm`'s disabled-while-submitting state; and `TaskList`'s tag-parsing and Cancelar path in `EditRow`. All added — 31 frontend tests now, all passing, still no regressions in the 99 backend tests.

The `board_tasks.status`/`.tags` dead columns and the personal `tasks` table's missing owner column, both flagged in the same review, were left alone — they're real but smaller, and don't carry the same "nothing catches a regression" risk that no tests at all did.
