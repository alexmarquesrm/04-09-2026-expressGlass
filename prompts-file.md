...
### Entry 22 — a "what's missing" review turns into frontend tests + CI - 2026-09-06
...
**Result obtained (summary):**
The review itself was grounded in the actual repo rather than memory of earlier sessions - `git log`, the migrations directory, every backend/frontend source file, and a grep for whether `status`/`tags` were still read anywhere. That turned up one gap worth acting on immediately: 99 backend tests existed and ran nowhere automatically, and the frontend had zero tests of its own - every UI check this project ever had was a manual or throwaway Playwright session, which means nothing would catch a regression in, say, the assignee picker built two slices ago. Added Vitest + React Testing Library to the frontend (23 tests: the avatar-gradient/initials pure functions, `TaskForm`, `TaskList` including its delete-confirmation dialog, and `TaskModal`'s assignee picker and label toggles), and a two-job GitHub Actions workflow running both suites - backend against a real `postgres:16-alpine` service container, frontend standalone - on every push and pull request.

**Accepted / Corrected:**
One test failed on the first run, and it was a test bug, not a component bug: after adding a second assignee through the picker, the new person's name appears twice on screen at once (once as a chip, once still inside the open picker list with a checkmark), so `getByText` correctly refused to guess which one and threw a multiple-elements error. Switched to asserting the name appears at all (`getAllByText(...).length > 0`) rather than assuming single-occurrence text, which is the more honest assertion anyway - the test only cares that the assignment took effect, not how many places currently render the name.

**Why:**
This is the same "the review pass catches what feature-level work never will" pattern as Entry 14, just applied to test infrastructure instead of application code: no single feature slice was ever going to justify stopping to add a whole test framework, but a step back to ask "what's missing across the *whole* project" made the absence obvious immediately. Also notable operationally: with no Node/npm on the host machine at all, every install and test run for this entry went through `docker compose exec frontend ...` against the already-running dev container - the same "run it for real, don't trust it by inspection" discipline this project has followed since the very first SQL-casting bug, just applied to tooling setup instead of application logic this time.

**AI mistake or oddity noticed (this time found by Review-QA, not by running anything)?**
Yes, and it's a good example of a bug hiding behind a confidently-worded test. `TaskForm.test.jsx`'s first version asserted `title: '  Comprar leite  '` - untrimmed, padding intact - in a test literally named "submits the *trimmed* title". The assertion matched what the component actually did, so the test passed; the test's own name was the only thing saying it should have failed. `TaskForm.jsx` trimmed `description` before sending it but never trimmed `title`, unlike `TaskModal.jsx`, which trims both - an inconsistency nobody had reason to notice because nothing compared the two components' behavior side by side until a reviewer read both. Fixed the component, then fixed the test to assert what its own name promised. The lesson: a test whose name and assertion silently disagree is worse than no test at all, because it launders a real bug into something that reads as "covered" - reviewing new tests against the component they test (not just running them) is exactly the check that catches this, and it's why Review-QA read `TaskModal.jsx` line-by-line against `TaskModal.test.jsx` rather than only checking that `npm test` was green.
