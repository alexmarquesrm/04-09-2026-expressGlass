...
- **`frontend`** — the React UI (Vite dev server) on [http://localhost:5173](http://localhost:5173), the interface you actually click through: [http://localhost:5173/](http://localhost:5173/) for the task list (already in Portuguese) and [http://localhost:5173/boards](http://localhost:5173/boards) for the Trello-style boards. The natural-language assistant is the floating button in the bottom-right corner, available on every page once you're logged in (needs `DEEPSEEK_API_KEY` set to actually respond).
...
- **`frontend`** — a interface React (servidor de desenvolvimento Vite) em [http://localhost:5173](http://localhost:5173): [http://localhost:5173/](http://localhost:5173/) para a lista de tarefas (já em português) e [http://localhost:5173/boards](http://localhost:5173/boards) para os quadros ao estilo Trello. O assistente de conversa em linguagem natural é o botão flutuante no canto inferior direito, disponível em qualquer página depois de iniciar sessão (precisa de `DEEPSEEK_API_KEY` definida para responder de facto).
